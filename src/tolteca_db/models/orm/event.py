...
class EventLog(Base):
    ...
    All state transitions emit events here for replay, debugging, and
    compliance.
    ...
    event_type : str
        Event type (e.g. "FlagAdded", "FileMissing", "StatusChanged").
    entity_type : str
        Entity type ("product", "source", "location", etc.).
    entity_id : str
        Entity identifier.
    payload : dict | None
        Event payload (JSON).
