...
class DataProductStorage(Base):
    ...
    product_id: Mapped[str] = mapped_column(
        String(128),
        ForeignKey("data_product.product_id"),
        primary_key=True
    )
    ...
    __table_args__ = (
        Index('ix_storage_location_role', 'location_id', 'role'),
    )
...
class DataProductFlag(Base):
    ...
    Composite primary key (product_id, flag_key).
    ...
    __table_args__ = (
        Index('ix_flag_product_flagkey', 'flag_key', 'product_id'),
    )
...
class DataProductAssoc(Base):
    ...
    assoc_id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    
    # Use DataProdAssocType enum for TolTEC associations
    assoc_type: Mapped[str] = mapped_column(String(32), index=True)
    
    src_product_id: Mapped[str] = mapped_column(
        String(128),
        ForeignKey("data_product.product_id"),
        index=True
    )
    
    dst_product_id: Mapped[str] = mapped_column(
        String(128),
        ForeignKey("data_product.product_id"),
        index=True
    )
    ...
    __table_args__ = (
        UniqueConstraint('assoc_type', 'src_product_id', 'dst_product_id', name='uq_assoc_edge'),
        Index('ix_assoc_src_type', 'assoc_type', 'src_product_id'),
        Index('ix_assoc_dst_type', 'assoc_type', 'dst_product_id'),
    )
...
class TaskInput(Base):
    ...
    Composite primary key (task_id, product_id).
    """
    ...
    task_id: Mapped[str] = mapped_column(
        String(64),
        ForeignKey("reduction_task.task_id"),
        primary_key=True
    )
