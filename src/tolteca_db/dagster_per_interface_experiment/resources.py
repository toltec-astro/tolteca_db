...
class ToltecaDBResource(ConfigurableResource):
    ...
    def _effective_database_url(self) -> str:
        ...
        In test mode, override database_url to use shared test file.
        """
        import os
        ...
        if os.getenv("DAGSTER_TEST_MODE") == "1":
            ...
            test_db_path = Path(".dagster/test_tolteca.duckdb")
            return f"duckdb:///{test_db_path}"
    ...
    def setup_for_execution(self, context) -> None:
        ...
        # In test mode, always clear database on startup for fresh state
        if os.getenv("DAGSTER_TEST_MODE") == "1":
            test_db_path = Path(".dagster/test_tolteca.duckdb")
            ...
            if test_db_path.exists():
                test_db_path.unlink()
                context.log.info(f"✓ Cleared previous test database at {test_db_path}")
                context.log.info(f"Creating shared test database at {test_db_path}")
    ...
    def get_session(self) -> Session:
        ...
        engine = get_engine(database_url=self._effective_database_url)
